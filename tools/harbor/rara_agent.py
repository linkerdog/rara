...
DEFAULT_STDERR_PATH = EnvironmentPaths.agent_dir / "rara-exec.stderr"
...
class RaraAgent(BaseInstalledAgent):
    ...
    def _build_exec_command(self) -> str:
        ...
        stderr_path = shlex.quote(DEFAULT_STDERR_PATH.as_posix())
        ...
        return (
            ...
            f"< {instruction_path} 2> {stderr_path}; "
            f"printf '%s\\n' \"$?\" > {status_path}; "
            f"}} | tee {jsonl_path}; "
            f"status=$(cat {status_path} 2>/dev/null || printf '1'); "
            f"if [ \"$status\" != \"0\" ]; then cat {stderr_path} >&2; fi; "
            'exit "$status"'
        )
    ...
    def _populate_context(context: AgentContext, events: list[dict[str, Any]]) -> None:
        ...
        context.metadata = {
            ...
            "stderr_path": DEFAULT_STDERR_PATH.as_posix(),
