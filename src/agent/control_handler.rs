...
use crate::agent::{Agent, AgentOutputMode, AgentExecutionMode, BashApprovalDecision, AgentEvent};
...
impl Agent {
    ...
    pub async fn handle_input_control<F>(&mut self, request: &InputControlRequest, report: F) -> Result<()> 
    where
        F: FnMut(AgentEvent) + Send,
    {
        match request {
            InputControlRequest::SubmitUserPrompt { prompt } => {
                self.query_with_mode_and_events(prompt.clone(), AgentOutputMode::Silent, report).await?;
            }
            InputControlRequest::AnswerPendingInput { answer } => {
                self.consume_pending_user_input(answer);
                self.query_with_mode_and_events(answer.clone(), AgentOutputMode::Silent, report).await?;
            }
            InputControlRequest::AnswerPlanApproval { approved } => {
                if *approved {
                    self.set_execution_mode(AgentExecutionMode::Execute);
                    self.query_with_mode_and_events("Plan approved. Proceed with implementation.".to_string(), AgentOutputMode::Silent, report).await?;
                } else {
                    self.query_with_mode_and_events("Plan rejected. Please revise the plan or suggest an alternative.".to_string(), AgentOutputMode::Silent, report).await?;
                }
            }
            InputControlRequest::AnswerShellApproval { decision } => {
                let decision = BashApprovalDecision::from(*decision);
                self.answer_pending_approval_with_events(decision, AgentOutputMode::Silent, report).await?;
            }
